 ...
 * 
 */
	private Assertion handleEncryptedAssertions(
			BasicSAMLMessageContext context, AuthnRequest request,
			Response response, Assertion subjectAssertion)
	 ...
	 * 
	 */
	private Assertion handleRegularAssertions(
			BasicSAMLMessageContext context, AuthnRequest request,
			Response response, Assertion subjectAssertion)
			throws SAMLException, SecurityException, ValidationException {
     ...
     */
    protected boolean verifyAssertionSignature(Signature signature, BasicSAMLMessageContext context) throws SAMLException, org.opensaml.xml.security.SecurityException, ValidationException {
  ...
  return decryptSAMLAssertion(
		  encryptedAssertion, decrypter); 
   ...
   * 
   */
  private Assertion decryptSAMLAssertion(
		  EncryptedAssertion encryptedAssertion, Decrypter decrypter) {
    ...
    protected boolean verifySignature(Signature signature, String IDPEntityID) throws org.opensaml.xml.security.SecurityException, ValidationException {
  ...
  log.debug("Authentication instant is withing MAX_AUTHENTICATION_TIME");
    ...
    private boolean isDateTimeSkewValid(int skewInSec, DateTime time, String descriptor) {
    	...
    	log.debug("Comparing current_time_stamp:  " + current_dt_utcValue.toString() + " " + descriptor + " timestamp:  " + time.toString());
    	log.debug("Time window in seconds:  " + skewInSec);
    	log.debug("Validating  timestamp is " + skewInSec + " before " + current_dt_utcValue.toString() + " OR " + skewInSec + " after "   + current_dt_utcValue.toString());
